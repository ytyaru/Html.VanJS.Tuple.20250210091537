...
//            if (!o.type){throw new TypeError(`値が未設定です。TupleはTuple.of,Type.mutメソッドの第一引数(文字列型)にてその要素を定義します。一つ以上の要素を持ち、それらはスペース、カンマ、セミコロン、改行のいずれかによって区切られます。要素は「key:type=value」で定義します。typeは任意であり最低でも「key=value」形式で要素を定義せねばなりません。:${script}`)}
    ...
    static #parseMultiLine(script) {
        ...
        //const valueTxts = Math.max(0,typeIdx,defIdx)+1 < tsv.length ? tsv[tsv.length-1] : Array.of(...tsv[0]).map(v=>'')
...
//        console.log(typeIdx, defIdx)
        ...
        /*
        const types = -1===typeIdx[0]
            ? (-1===defIdx[0]
//                ? valueTxts.map((v,i)=>this.#getLiteralType(valueTxts,i,valueTxts[i]))
//                : defTxts.map((d,i)=>this.#getLiteralType(defTxts,i,defTxts[i])))
                //? valueTxts.map((v,i)=>this.#getLiteralType(valueTxts[i],valueTxts[i]))
                ? (-1===valueIdx
                    ? valueTxts
                    : valueTxts.map((v,i)=>this.#getLiteralType(valueTxts[i],valueTxts[i])))
                : defTxts.map((d,i)=>this.#getLiteralType(defTxts[i],defTxts[i])))
            : typeTxts.map((t,i)=>TypeParser.getType(t,defTxts[i]) ?? this.#getLiteralType(valueTxts[i],defTxts[i]))
            //: typeTxts.map((t,i)=>TypeParser.getType(t,defTxts[i]) ?? this.#getLiteralType(valueTxts,i,defTxts[i]))
        */
    ...
    static #getTypes(typeTxts, typeIdx, defTxts, defIdx, valueTxts, valueIdx) {
        ...
        if (-1!==typeIdx) {return typeTxts.map((t,i)=>TypeParser.getType(t,defTxts[i]) ?? this.#getLiteralType(valueTxts[i],defTxts[i]))}
                 ...
                 if (-1!==valueIdx) {return valueTxts.map((v,i)=>this.#getLiteralType(valueTxts[i],valueTxts[i]))}
            else if (-1!==defIdx) {return defTxts.map((d,i)=>this.#getLiteralType(defTxts[i],defTxts[i]))}
            else {return tsv[0].map(v=>new AnyDataType())}
        }
        */
    ...
    //static #getLiteralType(value, defTxts) {return (''===value && ''===defTxts) ? new AnyDataType(defTxts[i]) : LiteralType.get(value,defTxts)}
    //static #getLiteralType(rows, colIdx, defTxts) {return (''===row[colIdx] && ''===defTxts) ? new AnyDataType(defTxts[i]) : LiteralType.get(row[colIdx],defTxts)}
    /*
